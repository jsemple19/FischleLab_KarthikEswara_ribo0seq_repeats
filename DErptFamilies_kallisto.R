...
for(i in 1:length(samplesheet$sample)){
  ...
  tmpCnts<-read.delim(paste0(workDir,"/kallisto/",sample,"/abundance.tsv"),header=T)
  tmpCnts_sample<-data.frame(gene_id=tmpCnts$target_id, sample=tmpCnts$est_counts)
...
rpts<-countsTable[grepl("_rpt.*$",countsTable$gene_id),]
