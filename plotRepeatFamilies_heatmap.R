...
sigGenes<-unique(results$gene_id[results$sig])

resSig<-results[results$gene_id %in% sigGenes,]

resWide<-pivot_wider(resSig,id_cols=c("gene_id","class","type","rptAnnCount"),names_from="strain",values_from="log2FoldChange")

resWide<-resWide[,c("gene_id","class","type","rptAnnCount","Btm_vs_N2", "hpl2tm_vs_N2","lin61tm_vs_N2", "EM38_vs_N2", "EM90_vs_EM88",  "EM92_vs_EM88","EM91_vs_EM88")]
...
rowOrder<-order(resWide$class,resWide$type,resWide$gene_id, resWide$hpl2tm_vs_N2)
resWide<-resWide[rowOrder,]
#resWide$type<-factor(resWide$type,levels=unique(resWide$type))

mat<-as.matrix(resWide[,c("Btm_vs_N2", "hpl2tm_vs_N2","lin61tm_vs_N2", "EM38_vs_N2", "EM90_vs_EM88",  "EM92_vs_EM88", "EM91_vs_EM88")])
...
ht<-Heatmap(mat,cluster_columns = F, cluster_rows=F, name="LFC",
